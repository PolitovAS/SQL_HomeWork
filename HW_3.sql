...
SELECT post, COUNT(post) AS 'Количество сотрудников от 24 до 49'
FROM staff
...
# 8. Выведите id сотрудников, которые напечатали более 500 страниц за все дни;
SELECT staff_id, SUM(count_pages)
FROM activity_staff
GROUP BY staff_id
HAVING  SUM(count_pages) > 500;

# 9. Выведите дни, когда работало более 3 сотрудников. Также укажите кол-во сотрудников, которые работали в выбранные дни;
SELECT date_activity, COUNT(staff_id) AS 'Количество работников'
FROM activity_staff
GROUP BY date_activity
HAVING COUNT(staff_id) > 3;

# 10. Выведите среднюю заработную плату по должностям, которая составляет более 30000.
SELECT post, AVG(salary) AS 'Средняя заработнаю плата'
FROM staff
GROUP BY post
