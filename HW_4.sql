...
CREATE TABLE `shops` (
	`id` INT,
    `shopname` VARCHAR (100), PRIMARY KEY (id)
);
...
WHERE c.name = "Murzik";
-- 2
SELECT s.*, info_murzik.*
FROM shops s
JOIN
(SELECT name, shops_id
FROM cats WHERE name = "Murzik") AS info_murzik
ON s.id = info_murzik.shops_id;
...
SELECT a.an_name 'Название анализа', a.an_price 'Цена'
FROM Analysis a
